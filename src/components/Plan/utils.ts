...
export function parseSvg(svgInString: string): null | SvgParsedData {
    ...
    if (gcpElements) {
        const referencePoints: ReferencePoint[] = Array.from(gcpElements).map((gcp) => ({
        ...
        const realDistance = getRealDistance(referencePoints[1], referencePoints[2])
        const svgDistance = calculateDistance(referencePoints[1], referencePoints[2])
        const angle = calculateAngle(referencePoints[0], referencePoints[2], referencePoints[1])
...
function calculateDistance(pointA: ReferencePoint, pointB: ReferencePoint): number {
...
export function getRealDistance(pointA: ReferencePoint, pointB: ReferencePoint) {
    return haversineDistance(pointA.xReal, pointA.yReal, pointB.xReal, pointB.yReal);
}
